...
const shiftedIso=(localDateTime:string,minutes:number)=>new Date(parseLocalIso(localDateTime).getTime()+minutes*60000).toISOString().slice(0,19);
const changedPillars=(baseline:PillarSnapshot,current:PillarSnapshot):Pillar['labelCode'][]=>([['YEAR','year'],['MONTH','month'],['DAY','day'],['HOUR','hour']] as const).filter(([,key])=>baseline[key]!==current[key]).map(([code])=>code);

export function analyzeBirthTimeSensitivity(input:BirthInput,windowMinutes=120,stepMinutes=5):BirthTimeSensitivity {
  if(!Number.isInteger(windowMinutes)||windowMinutes<1||windowMinutes>720)throw baziError('SENSITIVITY_WINDOW','RangeError','windowMinutes phải là số nguyên trong 1..720','windowMinutes must be an integer in 1..720');
  if(!Number.isInteger(stepMinutes)||stepMinutes<1||stepMinutes>windowMinutes)throw baziError('SENSITIVITY_STEP','RangeError','stepMinutes phải là số nguyên trong 1..windowMinutes','stepMinutes must be an integer in 1..windowMinutes');
  const offsets=new Set<number>([-windowMinutes,0,windowMinutes]);for(let offset=-windowMinutes;offset<=windowMinutes;offset+=stepMinutes)offsets.add(offset);
  if(offsets.size>289)throw baziError('SENSITIVITY_LIMIT','RangeError','Phân tích hỗ trợ tối đa 289 mẫu; hãy tăng stepMinutes','Sensitivity analysis supports at most 289 samples; increase stepMinutes');
  const baselineChart=calculateBazi(input),baseline=pillarSnapshot(baselineChart.pillars),states=new Map<string,{firstOffsetMinutes:number;lastOffsetMinutes:number;localDateTime:string;pillars:PillarSnapshot}>();
