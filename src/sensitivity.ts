...
export function analyzeBirthTimeSensitivity(input:BirthInput,windowMinutes=120,stepMinutes=5):BirthTimeSensitivity {
  if(!Number.isInteger(windowMinutes)||windowMinutes<1||windowMinutes>720)throw new RangeError('windowMinutes phải là số nguyên trong 1..720');
  if(!Number.isInteger(stepMinutes)||stepMinutes<1||stepMinutes>windowMinutes)throw new RangeError('stepMinutes phải là số nguyên trong 1..windowMinutes');
  ...
  if(offsets.size>289)throw new RangeError('Phân tích hỗ trợ tối đa 289 mẫu; hãy tăng stepMinutes');
