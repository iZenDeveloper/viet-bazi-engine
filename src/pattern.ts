...
import type { Element, ElementScore, PatternAnalysis, Pillar, Stem } from './types.js';

export function analyzePattern(day:Stem,month:Pillar,elements:ElementScore[]):PatternAnalysis {
  const resource=ELEMENTS.find(e=>GENERATES[e]===day.element)!;
  const support=new Set<Element>([day.element,resource]);
