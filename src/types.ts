...
export interface LocalizedTimelinePillar {stemCode:StemCode;stem:string;branchCode:BranchCode;branch:string;text:string}
export interface LocalizedAnnualTimelineEntry {year:number;annual:LocalizedTimelinePillar;tenGodCode:TenGodCode;tenGod:string;activeLuck:{order:number;pillar:LocalizedTimelinePillar}|null}
export interface LocalizedAnnualTimelineReport {schemaVersion:'1.0';locale:'vi'|'en';fromYear:number;toYear:number;entries:LocalizedAnnualTimelineEntry[]}
...
export interface LocalizedMethodologyItem { code:string; text:string; value:string|boolean }
export interface LocalizedMethodologyReport { schemaVersion:'1.0'; locale:'vi'|'en'; profileCode:MethodologyManifest['profileCode']; engineVersion:string; items:LocalizedMethodologyItem[] }
export interface LocalizedChartPillar extends LocalizedTimelinePillar {labelCode:Pillar['labelCode'];label:string;tenGodCode:TenGodCode|'DAY_MASTER';tenGod:string}
...
export interface LocalizedAuditRule extends Omit<CalculationRuleTrace,'descriptionVi'> {text:string}
...
export type InterpretationFocus='overview'|'elements'|'career'|'relationships'|'timing';
export interface InterpretationPromptOptions {locale?:'vi'|'en';focus?:InterpretationFocus}
export interface InterpretationPromptBundle {
  schemaVersion:'1.0';templateCode:'GROUNDED_BAZI_INTERPRETATION';templateVersion:string;locale:'vi'|'en';focus:InterpretationFocus;
  messages:[{role:'system';content:string},{role:'user';content:string}];
  grounding:{summary:LocalizedChartSummary;facts:LocalizedFactsReport;methodology:LocalizedMethodologyReport;audit:LocalizedAuditReport};
}
...
export type EngineFeatureCode='FOUR_PILLARS'|'TRUE_SOLAR_TIME'|'TEN_GODS'|'HIDDEN_STEMS'|'ELEMENT_BALANCE'|'BRANCH_RELATIONS'|'LUCK_CYCLES'|'ANNUAL_ANALYSIS'|'SHEN_SHA'|'PATTERN_ANALYSIS'|'COMPATIBILITY'|'SVG_EXPORT'|'AUDIT_TRACE'|'BATCH'|'BIRTH_TIME_SENSITIVITY'|'WASM_CALENDAR'|'PROMPT_GROUNDING';
