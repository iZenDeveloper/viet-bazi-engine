...
export function parseLocalIso(value:string): Date {
  ...
  if (!m) throw new RangeError('localDateTime phải có dạng YYYY-MM-DDTHH:mm[:ss], không kèm múi giờ');
  const nums=m.slice(1).map(Number); const [y,mo,d,h,mi,s=0]=nums;
  const date=new Date(Date.UTC(y!,mo!-1,d!,h!,mi!,s));
  if (date.getUTCFullYear()!==y||date.getUTCMonth()!==mo!-1||date.getUTCDate()!==d||h!>23||mi!>59||s>59) throw new RangeError('Ngày giờ sinh không hợp lệ');
  return date;
}
...
export function solarCorrectionMinutes(date:Date,longitude:number,offsetMinutes:number):number {
  return 4*(longitude-offsetMinutes/4)+equationOfTime(date);
}
export function sexagenaryDayIndex(localSolar:Date,dayBoundary:DayBoundaryConvention='early-zi'):number {
...
export function findJieBoundary(utc:Date,direction:1|-1,longitude:(date:Date)=>number):Date {
  ...
  for(let i=0;i<40*24;i++) {
    previous=cursor;cursor=new Date(cursor.getTime()+direction*3600000);
    if(bucket(cursor)!==origin){let lo=Math.min(cursor.getTime(),previous.getTime()),hi=Math.max(cursor.getTime(),previous.getTime()),lowBucket=bucket(new Date(lo));while(hi-lo>1000){const mid=Math.floor((lo+hi)/2);if(bucket(new Date(mid))===lowBucket)lo=mid;else hi=mid;}return new Date(Math.round(hi/60000)*60000);}
  }
...
export function solarTermBoundary(year:number,targetLongitude:number):Date {
  ...
  for(let minutes=-5*1440;minutes<=5*1440;minutes+=10){const candidate=new Date(center+minutes*60000);const error=Math.abs(mod(solarLongitude(candidate)-targetLongitude+180,360)-180);if(error<bestError){bestError=error;best=candidate;}}
  let lo=best.getTime()-20*60000,hi=best.getTime()+20*60000;
  for(let i=0;i<24;i++){const m1=lo+(hi-lo)/3,m2=hi-(hi-lo)/3;const e1=Math.abs(mod(solarLongitude(new Date(m1))-targetLongitude+180,360)-180),e2=Math.abs(mod(solarLongitude(new Date(m2))-targetLongitude+180,360)-180);if(e1<e2)hi=m2;else lo=m1;}
  return new Date(Math.round((lo+hi)/2/60000)*60000);
}
export { mod };
import type { DayBoundaryConvention } from './types.js';
