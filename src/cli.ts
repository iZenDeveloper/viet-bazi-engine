...
try {
  ...
  if(!args.includes('--svg')&&(localeAt>=0&&!args.includes('--facts')&&!args.includes('--methodology')&&!args.includes('--compatibility')&&!args.includes('--audit')&&timelineAt<0&&sensitivityAt<0||titleAt>=0||widthAt>=0||args.includes('--no-hidden-stems')||args.includes('--no-element-balance')||args.includes('--high-contrast')))throw new TypeError('--locale chỉ dùng cùng --svg, --facts, --methodology, --compatibility, --audit, --timeline hoặc --sensitivity');
