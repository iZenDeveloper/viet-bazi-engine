...
try {
  ...
  if(!json)throw new TypeError('Usage: viet-bazi [--stdin] [--batch|--compatibility|--audit|--summary|--sensitivity 120:5|--svg] [--year 2026] [--timeline 2025:2035] [--compact] JSON');
  ...
  if(args.includes('--summary')&&(args.includes('--batch')||args.includes('--compatibility')||args.includes('--audit')||args.includes('--facts')||args.includes('--methodology')||args.includes('--svg')||sensitivityAt>=0||timelineAt>=0))throw new TypeError('--summary không dùng cùng calculation mode khác');
  ...
  if(!args.includes('--svg')&&(localeAt>=0&&!args.includes('--facts')&&!args.includes('--methodology')&&!args.includes('--compatibility')&&!args.includes('--audit')&&!args.includes('--summary')&&timelineAt<0&&sensitivityAt<0||titleAt>=0||widthAt>=0||args.includes('--no-hidden-stems')||args.includes('--no-element-balance')||args.includes('--high-contrast')))throw new TypeError('--locale chỉ dùng cùng --svg, --facts, --methodology, --compatibility, --audit, --summary, --timeline hoặc --sensitivity');
