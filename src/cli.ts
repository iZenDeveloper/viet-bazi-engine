#!/usr/bin/env node
import { analyzeBirthTimeSensitivityFromJson,calculateBaziBatchFromJson,calculateBaziFromJson,compareBirthInputsFromJson,createBaziAuditReportFromJson,localizeCompatibilityFromJson,localizeFactsFromJson,localizeMethodologyFromJson,renderBaziSvgFromJson } from './json.js';
...
try {
  ...
  if(!args.includes('--svg')&&(localeAt>=0&&!args.includes('--facts')&&!args.includes('--methodology')&&!args.includes('--compatibility')||titleAt>=0||widthAt>=0||args.includes('--no-hidden-stems')||args.includes('--no-element-balance')||args.includes('--high-contrast')))throw new TypeError('--locale chỉ dùng cùng --svg, --facts, --methodology hoặc --compatibility');
  ...
  const chart=args.includes('--batch')?calculateBaziBatchFromJson(json):args.includes('--compatibility')?(locale?localizeCompatibilityFromJson(json,locale):compareBirthInputsFromJson(json)):args.includes('--audit')?createBaziAuditReportFromJson(json,year):args.includes('--facts')?localizeFactsFromJson(json,locale==='en'?'en':'vi',year):args.includes('--methodology')?localizeMethodologyFromJson(json,locale==='en'?'en':'vi',year):sensitivityOptions?analyzeBirthTimeSensitivityFromJson(json,...sensitivityOptions,year):calculateBaziFromJson(json,year);
