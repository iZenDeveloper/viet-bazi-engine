...
try {
  ...
  if(!args.includes('--svg')&&(localeAt>=0&&!args.includes('--facts')&&!args.includes('--methodology')||titleAt>=0||widthAt>=0||args.includes('--no-hidden-stems')||args.includes('--no-element-balance')||args.includes('--high-contrast')))throw new TypeError('Các tùy chọn hiển thị SVG chỉ dùng cùng --svg, --facts hoặc --methodology');
