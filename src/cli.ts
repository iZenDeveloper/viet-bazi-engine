#!/usr/bin/env node
import { analyzeBirthTimeSensitivityFromJson,calculateBaziBatchFromJson,calculateBaziFromJson,compareBirthInputsFromJson,createBaziAuditReportFromJson,createInterpretationPromptFromJson,localizeBaziAuditReportFromJson,localizeChartSummaryFromJson,localizeCompatibilityFromJson,localizeFactsFromJson,localizeMethodologyFromJson,renderBaziSvgFromJson } from './json.js';
...
const args=process.argv.slice(2),yearAt=args.indexOf('--year'),timelineAt=args.indexOf('--timeline'),sensitivityAt=args.indexOf('--sensitivity'),localeAt=args.indexOf('--locale'),focusAt=args.indexOf('--focus'),titleAt=args.indexOf('--title'),widthAt=args.indexOf('--width'),year=yearAt>=0?Number(args[yearAt+1]):undefined;
const excluded=new Set<number>();for(const at of [yearAt,timelineAt,sensitivityAt,localeAt,focusAt,titleAt,widthAt])if(at>=0){excluded.add(at);excluded.add(at+1);}
const inlineJson=args.find((x,i)=>!excluded.has(i)&&!['--compact','--error-json','--batch','--compatibility','--audit','--facts','--methodology','--summary','--prompt','--stdin','--svg','--no-hidden-stems','--no-element-balance','--high-contrast'].includes(x));
try {
  ...
  if(!json)throw new TypeError('Usage: viet-bazi [--stdin] [--batch|--compatibility|--audit|--summary|--prompt --focus overview|--sensitivity 120:5|--svg] [--year 2026] [--timeline 2025:2035] [--compact] JSON');
  ...
  if(args.includes('--prompt')&&(args.includes('--batch')||args.includes('--compatibility')||args.includes('--audit')||args.includes('--facts')||args.includes('--methodology')||args.includes('--summary')||args.includes('--svg')||sensitivityAt>=0||timelineAt>=0))throw new TypeError('--prompt không dùng cùng calculation mode khác');
  if(args.includes('--summary')&&(args.includes('--batch')||args.includes('--compatibility')||args.includes('--audit')||args.includes('--facts')||args.includes('--methodology')||args.includes('--prompt')||args.includes('--svg')||sensitivityAt>=0||timelineAt>=0))throw new TypeError('--summary không dùng cùng calculation mode khác');
  ...
  if(!args.includes('--svg')&&(localeAt>=0&&!args.includes('--facts')&&!args.includes('--methodology')&&!args.includes('--compatibility')&&!args.includes('--audit')&&!args.includes('--summary')&&!args.includes('--prompt')&&timelineAt<0&&sensitivityAt<0||titleAt>=0||widthAt>=0||args.includes('--no-hidden-stems')||args.includes('--no-element-balance')||args.includes('--high-contrast')))throw new TypeError('--locale chỉ dùng cùng --svg, --facts, --methodology, --compatibility, --audit, --summary, --prompt, --timeline hoặc --sensitivity');
  ...
  const focus=focusAt>=0?args[focusAt+1]:undefined;if(focus!==undefined&&!['overview','elements','career','relationships','timing'].includes(focus))throw new RangeError('--focus phải là overview, elements, career, relationships hoặc timing');if(focus!==undefined&&!args.includes('--prompt'))throw new TypeError('--focus chỉ dùng cùng --prompt');
