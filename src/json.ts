...
export function validateBirthInput(value:unknown,legacyAsOfYear?:number):BirthInput {
  ...
  if(x.solarTermModel!==undefined&&x.solarTermModel!=='legacy'&&x.solarTermModel!=='apparent'&&x.solarTermModel!=='ephemeris')throw baziError('SOLAR_TERM_MODEL','TypeError','solarTermModel phải là legacy, apparent hoặc ephemeris','solarTermModel must be legacy, apparent, or ephemeris');
