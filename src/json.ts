...
export function validateBirthInput(value:unknown,legacyAsOfYear?:number):BirthInput {
  ...
  if(x.solarTermModel!==undefined&&x.solarTermModel!=='legacy'&&x.solarTermModel!=='apparent')throw baziError('SOLAR_TERM_MODEL','TypeError','solarTermModel phải là legacy hoặc apparent','solarTermModel must be legacy or apparent');
