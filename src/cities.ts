import type { BirthInput, CityLocation } from './types.js';

export const VIETNAM_CITIES:readonly CityLocation[]=[
...
export function findCity(query:string):CityLocation|undefined {const q=normalize(query);return VIETNAM_CITIES.find(c=>[c.id,c.name,...c.aliases].some(x=>normalize(x)===q));}
export function resolveLocation(location:BirthInput['location']):{city?:string;latitude:number;longitude:number} {
  if(!location)throw new RangeError('trueSolarTime cần location');
  if(location.latitude!==undefined||location.longitude!==undefined){if(location.latitude===undefined||location.longitude===undefined)throw new RangeError('Phải truyền đủ latitude và longitude');if(!Number.isFinite(location.latitude)||location.latitude < -90||location.latitude > 90)throw new RangeError('latitude không hợp lệ');if(!Number.isFinite(location.longitude)||location.longitude < -180||location.longitude > 180)throw new RangeError('longitude không hợp lệ');return {latitude:location.latitude,longitude:location.longitude,...(location.city?{city:location.city}:{})};}
  if(!location.city)throw new RangeError('location cần city hoặc cặp latitude/longitude');const found=findCity(location.city);if(!found)throw new RangeError(`Không tìm thấy thành phố "${location.city}" trong catalog; hãy truyền tọa độ thủ công`);return {city:found.name,latitude:found.latitude,longitude:found.longitude};
}
