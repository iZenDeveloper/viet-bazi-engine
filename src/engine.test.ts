...
describe('calculateBazi',()=>{
  it('is deterministic and returns four known pillars for a Jia-Zi reference day',()=>{
    ...
    expect(a.metadata.methodology).toMatchObject({engineVersion:'1.0.0-rc.2',profileCode:'VIET_BAZI_STANDARD_V1',calendar:{dayBoundary:'EARLY_ZI'}});
  ...
  it('uses ephemeris solar terms by default and retains explicit legacy output',()=>{const base={localDateTime:'1600-02-04T10:30:00',timezoneOffsetMinutes:0,asOfYear:1600,gender:'male' as const},ephemeris=calculateBazi(base),legacy=calculateBazi({...base,solarTermModel:'legacy'});expect(ephemeris.input.solarTermModel).toBe('ephemeris');expect(ephemeris.pillars.year.branch.code).toBe('ZI');expect(legacy.input.solarTermModel).toBe('legacy');expect(legacy.pillars.year.branch.code).toBe('HAI');expect(ephemeris.metadata.methodology.calendar.solarTermModel).toBe('JIE_EPHEMERIS_TABLE_V1');});
  ...
  it('exposes precise neighboring Jie and flags a near-boundary birth',()=>{
    ...
    expect(r.normalized.solarTerms).toMatchObject({modelUncertaintyMinutes:3,boundaryRisk:'model-sensitive'});
