...
describe('engine capability discovery',()=>{
  ...
  it('advertises actual public limits and schemas',()=>{const value=getEngineCapabilities();expect(value).toMatchObject({schemaVersion:'1.0',engineVersion:'0.21.0',offline:true,runtimeDependencyCount:0,limits:{batchRecords:1000,stdinBytes:10485760,sensitivitySamples:289,annualTimelineYears:201}});expect(Object.values(value.schemas).every(id=>id.startsWith('https://viet-bazi.dev/schema/'))).toBe(true);});
});
