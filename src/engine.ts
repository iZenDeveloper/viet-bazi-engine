...
export const ENGINE_VERSION='1.0.0-rc.1' as const;
export function getMethodologyManifest(dayBoundary:DayBoundaryConvention,trueSolarTime:boolean,solarTermModel:SolarTermModel='legacy'):MethodologyManifest {
...
function calculateCore(input:BirthInput|LegacyBirthInput,legacyAsOfYear:number|undefined,calendar:CalendarOperations):BaziResult {
  ...
  const dayBoundary=input.dayBoundary??'early-zi',solarTermModel=input.solarTermModel??'legacy',normalizedInput:BirthInput={...input,asOfYear:asOfYear!,dayBoundary,solarTermModel};
