...
export interface CalendarOperations {
  baziYear(utc:Date,model:SolarTermModel):number; findNextJie(utc:Date,direction:1|-1,model:SolarTermModel):Date;
  sexagenaryDayIndex(localSolar:Date,dayBoundary:DayBoundaryConvention):number; solarMonthIndex(utc:Date,model:SolarTermModel):number;
