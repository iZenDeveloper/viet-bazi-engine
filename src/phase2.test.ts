...
import { compareBaziCharts } from './compatibility.js';
...
describe('phase 2 building blocks',()=>{
  it('returns a bounded, explainable compatibility score',()=>{const r=compareBaziCharts(a,b);expect(r.score).toBeGreaterThanOrEqual(0);expect(r.score).toBeLessThanOrEqual(100);expect(r.factors.reduce((n,f)=>n+f.score,0)).toBe(r.score);expect(r.metadata.methodology).toBe('heuristic-v1');});
