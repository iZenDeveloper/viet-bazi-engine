...
export function compareBaziCharts(a, b) {
    const factors = [];
    const ae = a.dayMaster.element, be = b.dayMaster.element;
    ...
    const ap = Object.fromEntries(a.elements.map(e => [e.element, e.percent])), bp = Object.fromEntries(b.elements.map(e => [e.element, e.percent]));
    const aNeeds = a.pattern.favorableElements, bNeeds = b.pattern.favorableElements;
    const suppliedA = aNeeds.filter(e => bp[e] >= 18), suppliedB = bNeeds.filter(e => ap[e] >= 18);
    const complement = [...new Set([...suppliedA, ...suppliedB])];
    ...
    const aBranches = Object.values(a.pillars).map(p => p.branch.name), bBranches = Object.values(b.pillars).map(p => p.branch.name);
