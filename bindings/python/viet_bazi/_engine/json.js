...
export function validateBirthInput(value, legacyAsOfYear) {
    ...
    try {
        ...
        throw baziError('LOCAL_DATETIME_FORMAT', 'RangeError', 'localDateTime không đúng định dạng hoặc không hợp lệ', 'localDateTime has an invalid format or value');
    }
    if (typeof x.timezoneOffsetMinutes !== 'number' || !Number.isInteger(x.timezoneOffsetMinutes) || Math.abs(x.timezoneOffsetMinutes) > 840)
        throw baziError('TIMEZONE_OFFSET', 'RangeError', 'timezoneOffsetMinutes phải là số nguyên trong -840..840', 'timezoneOffsetMinutes must be an integer in -840..840');
    const asOfYear = x.asOfYear ?? legacyAsOfYear;
    if (typeof asOfYear !== 'number' || !Number.isInteger(asOfYear) || asOfYear < 1600 || asOfYear > 2400)
        throw baziError('AS_OF_YEAR', 'RangeError', 'asOfYear phải là số nguyên trong 1600..2400', 'asOfYear must be an integer in 1600..2400');
    if (x.gender !== 'male' && x.gender !== 'female')
        throw baziError('GENDER', 'TypeError', 'gender phải là male hoặc female', 'gender must be male or female');
    if (x.trueSolarTime !== undefined && typeof x.trueSolarTime !== 'boolean')
    ...
    if (x.location !== undefined) {
        if (!x.location || typeof x.location !== 'object' || Array.isArray(x.location))
            throw baziError('LOCATION_TYPE', 'TypeError', 'location phải là object', 'location must be an object');
        const l = x.location;
        assertKnownKeys(l, ['city', 'latitude', 'longitude'], 'location');
        if (l.latitude !== undefined && (typeof l.latitude !== 'number' || !Number.isFinite(l.latitude) || l.latitude < -90 || l.latitude > 90))
            throw baziError('LATITUDE', 'RangeError', 'latitude không hợp lệ', 'latitude must be in -90..90');
        if (l.longitude !== undefined && (typeof l.longitude !== 'number' || !Number.isFinite(l.longitude) || l.longitude < -180 || l.longitude > 180))
            throw baziError('LONGITUDE', 'RangeError', 'longitude không hợp lệ', 'longitude must be in -180..180');
        if ((l.latitude === undefined) !== (l.longitude === undefined))
            throw baziError('LOCATION_COORDINATES_REQUIRED', 'RangeError', 'Phải truyền đủ latitude và longitude', 'Both latitude and longitude are required');
        if (l.city !== undefined && (typeof l.city !== 'string' || l.city.length < 1))
            throw baziError('CITY_TYPE', 'TypeError', 'city phải là string không rỗng', 'city must be a non-empty string');
        if (l.city === undefined && l.latitude === undefined)
            throw baziError('LOCATION_REQUIRED', 'RangeError', 'location cần city hoặc cặp latitude/longitude', 'location requires a city or latitude/longitude pair');
        location = { ...(typeof l.latitude === 'number' ? { latitude: l.latitude } : {}), ...(typeof l.longitude === 'number' ? { longitude: l.longitude } : {}), ...(typeof l.city === 'string' ? { city: l.city } : {}) };
    }
...
export function calculateBaziFromJson(json, asOfYear) {
    ...
    try {
        ...
        throw baziError('INVALID_JSON', 'SyntaxError', 'JSON input không hợp lệ', 'Invalid JSON input');
    }
    return calculateBazi(validateBirthInput(parsed, asOfYear));
}
export function calculateBaziBatch(values) {
    if (!Array.isArray(values))
        throw baziError('BATCH_NOT_ARRAY', 'TypeError', 'Batch input phải là JSON array', 'Batch input must be a JSON array');
    if (values.length > 1000)
        throw baziError('BATCH_LIMIT', 'RangeError', 'Batch hỗ trợ tối đa 1000 records', 'Batch supports at most 1000 records');
    const items = values.map((value, index) => { try {
        return { index, ok: true, result: calculateBazi(validateBirthInput(value)) };
    }
    catch (error) {
        return { index, ok: false, error: toBaziErrorPayload(error) };
    } });
    const succeeded = items.filter(item => item.ok).length;
    return { schemaVersion: '1.1', summary: { total: items.length, succeeded, failed: items.length - succeeded }, items };
}
export function calculateBaziBatchFromJson(json) {
    let parsed;
    try {
        parsed = JSON.parse(json);
    }
    catch {
        throw baziError('INVALID_JSON', 'SyntaxError', 'JSON batch input không hợp lệ', 'Invalid JSON batch input');
    }
    if (!Array.isArray(parsed))
        throw baziError('BATCH_NOT_ARRAY', 'TypeError', 'Batch input phải là JSON array', 'Batch input must be a JSON array');
    return calculateBaziBatch(parsed);
}
export function analyzeBirthTimeSensitivityFromJson(json, windowMinutes = 120, stepMinutes = 5, asOfYear) {
    let parsed;
    try {
        parsed = JSON.parse(json);
    }
    catch {
        throw baziError('INVALID_JSON', 'SyntaxError', 'JSON sensitivity input không hợp lệ', 'Invalid JSON sensitivity input');
    }
...
export function compareBirthInputsFromJson(json) {
    ...
    try {
        ...
        throw baziError('INVALID_JSON', 'SyntaxError', 'JSON compatibility input không hợp lệ', 'Invalid JSON compatibility input');
    }
    if (!Array.isArray(parsed) || parsed.length !== 2)
        throw baziError('COMPATIBILITY_ARITY', 'TypeError', 'Compatibility input phải là array đúng 2 birth inputs', 'Compatibility input must be an array of exactly 2 birth inputs');
    return compareBirthInputs(validateBirthInput(parsed[0]), validateBirthInput(parsed[1]));
}
export function localizeCompatibilityFromJson(json, locale = 'vi') { return localizeCompatibility(compareBirthInputsFromJson(json), locale); }
...
export function localizeBaziAuditReportFromJson(json, locale = 'vi', asOfYear) { return localizeBaziAuditReport(createBaziAuditReportFromJson(json, asOfYear), locale); }
...
export function localizeAnnualTimelineFromJson(json, fromYear, toYear, locale = 'vi', asOfYear) { return localizeAnnualTimeline(calculateAnnualTimelineFromJson(json, fromYear, toYear, asOfYear), locale); }
