...
export function validateBirthInput(value, legacyAsOfYear) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        throw new TypeError('Input phải là JSON object');
    const x = value;
    ...
    if (typeof x.localDateTime !== 'string')
        throw new TypeError('localDateTime phải là string');
    ...
    if (typeof x.timezoneOffsetMinutes !== 'number' || !Number.isInteger(x.timezoneOffsetMinutes) || Math.abs(x.timezoneOffsetMinutes) > 840)
        throw new RangeError('timezoneOffsetMinutes phải là số nguyên trong -840..840');
    ...
    if (x.location !== undefined) {
        ...
        if (l.city !== undefined && (typeof l.city !== 'string' || l.city.length < 1))
            throw new TypeError('city phải là string không rỗng');
...
export function calculateBaziFromJson(json, asOfYear) {
    ...
    try {
        ...
        throw new SyntaxError('JSON input không hợp lệ');
    }
    return calculateBazi(validateBirthInput(parsed, asOfYear));
}
export function calculateBaziBatch(values) {
    if (!Array.isArray(values))
        throw new TypeError('Batch input phải là JSON array');
    if (values.length > 1000)
        throw new RangeError('Batch hỗ trợ tối đa 1000 records');
    const items = values.map((value, index) => { try {
        return { index, ok: true, result: calculateBazi(validateBirthInput(value)) };
    }
    catch (error) {
        return { index, ok: false, error: { name: error instanceof Error ? error.name : 'Error', message: error instanceof Error ? error.message : String(error) } };
    } });
...
export function calculateBaziBatchFromJson(json) {
    ...
    try {
        ...
        throw new SyntaxError('JSON batch input không hợp lệ');
    }
