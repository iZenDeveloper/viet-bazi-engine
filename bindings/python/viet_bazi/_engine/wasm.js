...
function operations(kernel) {
    const longitude = (date, model) => model === 'apparent' ? solarLongitudeApparent(date) : kernel.solarLongitude(date.getTime());
    return {
        baziYear(utc, model) { if (model === 'ephemeris')
            return baziYear(utc, model); const y = utc.getUTCFullYear(), lon = longitude(utc, model); return utc.getUTCMonth() < 2 && lon < 315 ? y - 1 : y; },
