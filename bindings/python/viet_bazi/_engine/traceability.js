const rule = (ruleCode, ruleVersion, category, descriptionVi, inputPaths, outputPaths) => ({ ruleCode, ruleVersion, category, descriptionVi, inputPaths, outputPaths });
/** Build a deterministic, machine-readable map from calculation rules to result fields. */
export function createBaziAuditReport(chart) {
    const rules = [
    ...
    if (chart.metadata.methodology.trueSolarTime.enabled)
        rules.splice(2, 0, rule('TRUE_SOLAR_TIME_LONGITUDE_EOT', '1.0', 'normalization', 'Giờ dân sự được hiệu chỉnh theo kinh độ và phương trình thời gian.', ['input.localDateTime', 'input.timezoneOffsetMinutes', 'input.location'], ['normalized.solarTime', 'normalized.correctionMinutes', 'normalized.location']));
