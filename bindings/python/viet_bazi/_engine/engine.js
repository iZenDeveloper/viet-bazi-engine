...
function tenGod(day, other) {
    ...
    if (other.element === day.element)
        return same ? 'Tỷ Kiên' : 'Kiếp Tài';
    if (GENERATES[day.element] === other.element)
        return same ? 'Thực Thần' : 'Thương Quan';
    if (CONTROLS[day.element] === other.element)
        return same ? 'Thiên Tài' : 'Chính Tài';
    if (CONTROLS[other.element] === day.element)
        return same ? 'Thất Sát' : 'Chính Quan';
    return same ? 'Thiên Ấn' : 'Chính Ấn';
}
function branch(i, day) {
...
function pillar(label, si, bi, day) { const s = stem(si), god = label === 'Ngày' ? 'Nhật Chủ' : tenGod(day, s); const labelCode = { Năm: 'YEAR', Tháng: 'MONTH', Ngày: 'DAY', Giờ: 'HOUR' }; return { labelCode: labelCode[label], label, stem: s, branch: branch(bi, day), tenGodCode: god === 'Nhật Chủ' ? 'DAY_MASTER' : TEN_GOD_CODES[god], tenGod: god }; }
const RELATION_CODES = { 'Lục hợp': 'LIU_HE', 'Lục xung': 'LIU_CHONG', 'Lục hại': 'LIU_HAI', 'Lục phá': 'LIU_PO', 'Tự hình': 'ZI_XING', 'Tam hình': 'SAN_XING', 'Tam hợp': 'SAN_HE', 'Tam hội': 'SAN_HUI' };
const makeRelation = (type, branches, resultElement) => ({ typeCode: RELATION_CODES[type], type, branchCodes: branches.map(x => BRANCH_CODES[x]), branches, ...(resultElement ? { resultElementCode: ELEMENT_CODES[resultElement], resultElement } : {}) });
function relations(names) {
    const out = [];
    const has = (...x) => x.every(n => names.includes(n));
    const pairs = [
    ...
    for (const [type, a, b] of pairs)
        if (has(a, b))
            out.push(makeRelation(type, [a, b]));
    for (const n of ['Thìn', 'Ngọ', 'Dậu', 'Hợi'])
        if (names.filter(x => x === n).length > 1)
            out.push(makeRelation('Tự hình', [n, n]));
    const punishments = [['Dần', 'Tỵ', 'Thân'], ['Sửu', 'Mùi', 'Tuất'], ['Tý', 'Mão']];
    for (const bs of punishments)
        if (has(...bs))
            out.push(makeRelation('Tam hình', bs));
    const triples = [['Tam hợp', ['Thân', 'Tý', 'Thìn'], 'Thủy'], ['Tam hợp', ['Hợi', 'Mão', 'Mùi'], 'Mộc'], ['Tam hợp', ['Dần', 'Ngọ', 'Tuất'], 'Hỏa'], ['Tam hợp', ['Tỵ', 'Dậu', 'Sửu'], 'Kim'], ['Tam hội', ['Hợi', 'Tý', 'Sửu'], 'Thủy'], ['Tam hội', ['Dần', 'Mão', 'Thìn'], 'Mộc'], ['Tam hội', ['Tỵ', 'Ngọ', 'Mùi'], 'Hỏa'], ['Tam hội', ['Thân', 'Dậu', 'Tuất'], 'Kim']];
    for (const [type, bs, e] of triples)
        if (has(...bs))
            out.push(makeRelation(type, bs, e));
    return out;
}
function elementScores(ps) {
    const score = Object.fromEntries(ELEMENTS.map(e => [e, 0]));
    for (const p of ps) {
        score[p.stem.element] += 1;
        score[p.branch.element] += .7;
        for (const h of p.branch.hiddenStems)
            score[h.element] += .8 * h.weight;
    }
    // Month branch carries seasonal qi.
    score[ps[1].branch.element] += 1.5;
...
function annualPillar(year, day) { const idx = mod(year - 1984, 60); return pillar('Năm', idx % 10, idx % 12, day); }
function luckPillars(month, birthYear, startAge, direction, day) { const base = month.stem.index; const bb = month.branch.index; return Array.from({ length: 8 }, (_, k) => { const order = k + 1, s = stem(base + direction * order), god = tenGod(day, s); return { order, startAge: Math.round((startAge + k * 10) * 10) / 10, startYear: birthYear + Math.ceil(startAge) + k * 10, stem: s, branch: branch(bb + direction * order, day), tenGodCode: TEN_GOD_CODES[god], tenGod: god }; }); }
function activeLuck(luckList, year) { const current = luckList.find(p => year >= p.startYear && year < p.startYear + 10) ?? null; return { asOfYear: year, order: current?.order ?? null, pillar: current }; }
function analyzeAnnual(day, natal, luckList, year) { const annual = annualPillar(year, day), god = annual.tenGod, active = activeLuck(luckList, year); return { year, pillar: annual, stemTenGodCode: TEN_GOD_CODES[god], stemTenGod: god, interactions: natal.map(p => ({ withPillarCode: p.labelCode, withPillar: p.label, relations: relations([annual.branch.name, p.branch.name]) })), activeLuckInteraction: active.pillar ? { order: active.pillar.order, relations: relations([annual.branch.name, active.pillar.branch.name]) } : null }; }
export function calculateAnnualTimeline(chart, fromYear, toYear) { if (!Number.isInteger(fromYear) || !Number.isInteger(toYear) || fromYear < 1600 || toYear > 2400 || fromYear > toYear)
...
export function calculateBaziWithCalendar(input, calendar) {
    return calculateCore(input, undefined, calendar);
}
function calculateCore(input, legacyAsOfYear, calendar) {
    ...
    const solar = new Date(civil.getTime() + correction * 60000), year = calendar.baziYear(utc, solarTermModel), yi = mod(year - 1984, 60), mi = calendar.solarMonthIndex(utc, solarTermModel), di = calendar.sexagenaryDayIndex(solar, dayBoundary);
    const day = stem(di % 10);
    const hourBranch = mod(Math.floor((solar.getUTCHours() + 1) / 2), 12);
    const hourStem = mod(day.index * 2 + hourBranch, 10);
    ...
    const facts = [{ code: 'DAY_MASTER', vi: `Nhật Chủ là ${day.name} ${day.element}.`, confidence: 'high', evidence: ['pillars.day.stem'] }, { code: 'SEASON', vi: `Sinh trong tháng ${monthP.branch.name}, khí mùa thiên về ${monthP.branch.element}.`, confidence: 'high', evidence: ['pillars.month.branch'] }, { code: 'ELEMENT_BALANCE', vi: `${strongest.element} nổi trội nhất (${strongest.percent}%), ${day.element} của Nhật Chủ ở mức ${dm.strength}.`, confidence: 'medium', evidence: ['elements'] }, ...(nearBoundary ? [{ code: 'NEAR_SOLAR_TERM', vi: `Thời điểm sinh cách ranh tiết gần nhất khoảng ${nearestDistanceMinutes} phút; mức bất định model là ${modelUncertaintyMinutes} phút.`, confidence: 'high', evidence: ['normalized.solarTerms'] }] : [])];
