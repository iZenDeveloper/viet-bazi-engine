...
for (const at of [yearAt, timelineAt, sensitivityAt, localeAt, focusAt, titleAt, widthAt])
    if (at >= 0) {
        excluded.add(at);
        excluded.add(at + 1);
    }
...
try {
    if (args.includes('--capabilities')) {
        ...
        if (!json)
            throw new TypeError('Usage: viet-bazi [--stdin] [--batch|--compatibility|--audit|--summary|--prompt --focus overview|--sensitivity 120:5|--svg] [--year 2026] [--timeline 2025:2035] [--compact] JSON');
        ...
        if (args.includes('--prompt') && (args.includes('--batch') || args.includes('--compatibility') || args.includes('--audit') || args.includes('--facts') || args.includes('--methodology') || args.includes('--summary') || args.includes('--svg') || sensitivityAt >= 0 || timelineAt >= 0))
            throw new TypeError('--prompt không dùng cùng calculation mode khác');
        if (args.includes('--summary') && (args.includes('--batch') || args.includes('--compatibility') || args.includes('--audit') || args.includes('--facts') || args.includes('--methodology') || args.includes('--prompt') || args.includes('--svg') || sensitivityAt >= 0 || timelineAt >= 0))
        ...
        const focus = focusAt >= 0 ? args[focusAt + 1] : undefined;
        if (focus !== undefined && !['overview', 'elements', 'career', 'relationships', 'timing'].includes(focus))
            throw new RangeError('--focus phải là overview, elements, career, relationships hoặc timing');
        if (focus !== undefined && !args.includes('--prompt'))
            throw new TypeError('--focus chỉ dùng cùng --prompt');
