...
class ClientTest(unittest.TestCase):
    def test_calculates_through_local_engine(self) -> None:
        ...
        self.assertEqual(result["metadata"]["methodology"]["engineVersion"], "1.0.0-rc.1")
    ...
    def test_creates_grounded_interpretation_prompt(self) -> None:
        result = create_interpretation_prompt(BirthInput("2000-01-07T12:00:00", 420, "male", 2026), locale="en", focus="elements")
        self.assertEqual(result["templateCode"], "GROUNDED_BAZI_INTERPRETATION")
        self.assertEqual(result["focus"], "elements")
        self.assertTrue(all(item["code"] and item["evidence"] for item in result["grounding"]["facts"]["facts"]))
    ...
    def test_verifies_bundled_engine_integrity(self) -> None:
        ...
        self.assertEqual(result, {"engineVersion": "1.0.0-rc.1", "files": 25, "verified": True})
