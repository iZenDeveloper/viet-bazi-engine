...
- Thêm manual Trusted Publishing workflow cho npm/PyPI bằng OIDC, environment approval và npm dist-tag `next` cho RC.
...
- Giữ `legacy` làm mặc định; model `apparent` khả dụng qua `solarLongitude()`, `solarTermBoundary()` và `BirthInput.solarTermModel`.
- Đồng bộ lựa chọn model qua JSON Schema/CLI/MCP, WASM orchestration và Python binding mà không đổi WASM ABI v1.
...
- Chuyển PWA navigation sang network-first và tự reload khi service worker mới takeover để tránh HTML/JS cache cũ làm live demo không chạy sau deploy.
