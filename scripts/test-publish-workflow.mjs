...
for(const token of ['workflow_dispatch:','tag:','default: v1.0.0-rc.2','contents: read','id-token: write','environment: registry-publish','node-version: 24','package-manager-cache: false','npm run release:check','Verify requested tag','npm view "$name@$version" version','npm publish --access public --tag "$dist_tag"','dist_tag=next','python -m pip wheel','pypa/gh-action-pypi-publish@release/v1','packages-dir: dist-pypi/'])if(!workflow.includes(token))throw new Error(`Publish workflow missing ${token}`);
