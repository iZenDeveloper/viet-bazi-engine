...
for(const path of ['dist/index.js','dist/index.d.ts','dist/wasm/calendar.wasm','schemas/manifest.json','fixtures/v1/manifest.json','fixtures/v1/jie-multi-year.json','fixtures/v1/timezone-boundaries.json','README.md','CHANGELOG.md','LICENSE'])if(!paths.has(path))throw new Error(`npm tarball missing ${path}`);
