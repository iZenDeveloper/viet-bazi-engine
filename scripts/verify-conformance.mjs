...
import { CONFORMANCE_VERSION,JIE_2026_FIXTURES,SEXAGENARY_DAY_FIXTURES,TIMEZONE_BOUNDARY_FIXTURES } from '../dist/conformance.js';

const root=resolve(dirname(fileURLToPath(import.meta.url)),'..');
const load=name=>JSON.parse(readFileSync(resolve(root,'fixtures/v1',name),'utf8'));
const jie=load('jie-2026.json'),days=load('sexagenary-days.json'),timezoneBoundaries=load('timezone-boundaries.json');
...
console.log(JSON.stringify({fixtureVersion:CONFORMANCE_VERSION,solarTerms:jie.length,sexagenaryDays:days.length,timezoneBoundaries:timezoneBoundaries.length,timezoneInvariantGroups:sameUtcGroups.size,maxSolarTermErrorMinutes:Math.round(maxSolarTermErrorMinutes*100)/100}));
