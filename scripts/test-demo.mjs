...
for(const token of ['datetime-local','trueSolarTime','calculateBazi','renderBaziSvg','VIETNAM_CITIES','dayBoundary','locale','download-json','nearBoundary','renderSummary','luck.active.pillar','pattern.primary','serviceWorker','cache.put','clients.claim','manifest.webmanifest','display','maskable'])if(!joined.includes(token))throw new Error(`Offline demo missing ${token}`);
